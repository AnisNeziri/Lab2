...
class ProductService
{
    ...
    public function delete(Product $product): void
    {
        DB::transaction(function () use ($product): void {
            $locked = Product::query()->lockForUpdate()->findOrFail($product->id);
            $warehouseQuantity = Schema::hasTable('warehouse_stock')
                ? (float) DB::table('warehouse_stock')->where('product_id', $locked->id)
                    ->lockForUpdate()
                    ->pluck('quantity')
                    ->sum()
                : 0.0;
            if (abs((float) $locked->quantity) >= 0.0005 || abs($warehouseQuantity) >= 0.0005) {
                throw ValidationException::withMessages([
                    'product' => ['A product with stock cannot be archived or deleted. Transfer, return, sell or count its stock to zero first.'],
                ]);
            }
    ...
    public function exportCsv(): StreamedResponse
    {
        ...
        $callback = function () use ($products) {
            ...
            fputcsv($handle, [
                ...
                'Supplier SKU', 'Supplier Currency', 'Exchange Rate to EUR', 'Exchange Rate Date',
                ...
                'Tracking Mode', 'Expiration Controlled', 'Default Shelf Life Days',
    ...
    {
        foreach (['barcode', 'brand', 'hs_code'] as $field) {
        ...
        if ($expirationControlled && ! in_array($trackingMode, ['batch', 'batch_expiry'], true)) {
            throw ValidationException::withMessages([
                'expiration_controlled' => ['Expiration-controlled stock must use lot/batch tracking.'],
            ]);
        }
        ...
        if (array_key_exists('lifecycle_status', $data)) {
            ...
            if ($status === 'active') {
                ...
                $warehouseQuantity = $existing && Schema::hasTable('warehouse_stock')
                    ? (float) DB::table('warehouse_stock')->where('product_id', $existing->id)
                        ->lockForUpdate()
                        ->pluck('quantity')
                        ->sum()
                    : 0.0;
                if (abs((float) ($existing?->quantity ?? 0)) >= 0.0005 || abs($warehouseQuantity) >= 0.0005) {
                    throw ValidationException::withMessages(['lifecycle_status' => ['A product with stock cannot be archived.']]);
                }
                $data['discontinued_at'] = $existing?->discontinued_at ?: now();
    ...
    private function assertBarcodeAvailability(int $companyId, ?string $primary, array $alternatives, ?int $productId = null): void
    {
        ...
        $normalized = $rows->pluck('barcode')->map(fn ($value) => mb_strtolower($value));
        if ($normalized->duplicates()->isNotEmpty() || ($primary !== '' && $normalized->contains(mb_strtolower($primary)))) {
            throw ValidationException::withMessages(['alternative_barcodes' => ['Every barcode must be unique for this product.']]);
        }

        $all = $rows->pluck('barcode')->when($primary !== '', fn ($values) => $values->push($primary))->all();
        if ($all === []) {
            return;
        }
    ...
    private function syncBarcodes(Product $product, array $rows): void
    {
        ...
        foreach ($barcodes as $row) {
            $product->alternativeBarcodes()->updateOrCreate(
                ['barcode' => $row['barcode']],
                ['company_id' => $product->company_id, 'label' => $row['label'], 'is_active' => $row['is_active']],
            );
        }
        $product->alternativeBarcodes()->whereNotIn('barcode', $barcodes->pluck('barcode')->all())->delete();
    }
