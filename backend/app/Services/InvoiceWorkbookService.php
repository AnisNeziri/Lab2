...
class InvoiceWorkbookService
{
    public function build(Invoice $invoice, string $locale = 'bilingual'): string
    {
        ...
        $summary = [
            ...
            [data_get($invoice->source_settlement, 'basis') === 'customer_ledger' ? ($locale === 'sq' ? 'Shlyer në regjistrin e klientit' : 'Settled in customer ledger') : $this->label('paid', $locale), OpenXmlWorkbook::cell($invoice->document_type === 'credit_note' ? 0 : (float) data_get($invoice->source_settlement, 'settled', $invoice->total_paid), 'currency')],
            [$this->label('remaining', $locale), OpenXmlWorkbook::cell((float) data_get($invoice->source_settlement, 'outstanding', $invoice->remaining_balance), 'currency')],
