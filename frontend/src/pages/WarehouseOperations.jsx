...
export default function WarehouseOperations() {
  ...
  return (
    <main className="warehouse-ops-page">
      ...
      {receiveTarget && <div className="modal-overlay" onMouseDown={(event) => event.target === event.currentTarget && !busy && setReceiveTarget(null)}><section className="modal warehouse-action-modal" role="dialog" aria-modal="true"><header className="modal-header"><h2>{t('warehouseOps.receive')} {receiveTarget.transfer_number}</h2><button className="modal-close-btn" disabled={busy} onClick={() => setReceiveTarget(null)}><X /></button></header><div className="modal-body"><p>{receiveTarget.destination_warehouse?.name}</p>{receiveLines.map((line, index) => <div className="receive-line" key={line.id}><strong>{line.name}</strong><small>{t('warehouseOps.remaining')}: {formatQuantity(line.remaining, line.unit)} {line.unit}</small><label>{t('warehouseOps.accepted')}<input type="number" min="0" max={line.remaining} step="0.001" value={line.accepted_quantity} onChange={(event) => setReceiveLines((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, accepted_quantity: event.target.value } : row))}/></label><label>{t('warehouseOps.damaged')}<input type="number" min="0" max={line.remaining} step="0.001" value={line.damaged_quantity} onChange={(event) => setReceiveLines((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, damaged_quantity: event.target.value } : row))}/></label></div>)}<button disabled={busy} onClick={() => perform(async () => { await receiveStockTransfer(receiveTarget.id, { idempotency_key: requestKey(), items: receiveLines.map((row) => ({ id: row.id, accepted_quantity: Number(row.accepted_quantity || 0), damaged_quantity: Number(row.damaged_quantity || 0) })) }); setReceiveTarget(null) }, t('warehouseOps.receivedSaved'))}>{t('warehouseOps.confirmReceipt')}</button></div></section></div>}
