...
import {
  ...
  getSupplierPerformance, moveBinStock, postLandedCost, recalculateSupplierInvoice, reverseLandedCost,
...
const tabPermissions = { locator:'inventory.view', counts:'inventory.counts.manage', expiry:'inventory.view', replenish:'replenishment.view', catalogue:'supplier_catalogue.view', landed:'landed_costs.manage', returns:'returns.view', matching:'supplier_invoices.view' };
...
export default function OperationsCenter() {
 const tx = useUiText()
  ...
  const tabFromUrl = (params) => params.has('return') ? 'returns' : ({replenishment:'replenish','landed-costs':'landed'}[params.get('tab')] || params.get('tab') || 'locator');
  const permittedTab = value => can(tabPermissions[value]) ? value : Object.keys(tabPermissions).find(id => can(tabPermissions[id]));
  const [tab, setTab] = useState(() => permittedTab(tabFromUrl(pageParams)));
  ...
  const [landedReverseId, setLandedReverseId] = useState("");

  const loadMeta = useCallback(async () => {
    const safe = (promise) => promise.catch(() => []);
    const [products, suppliers, warehouses, customers, orders, receipts, accounts] = await Promise.all([
      safe(getAllProducts()), safe(getSuppliers()), safe(getWarehouses()), can('debts.view') ? safe(collectPages(getCustomerDebts)) : [],
      can('purchase_orders.view') ? safe(collectPages(getPurchaseOrders)) : [], safe(collectPages(getGoodsReceipts)), can('financial_accounts.view') ? safe(getFinancialAccounts()) : [],
    ]);
  ...
  const reload = useCallback(async () => {
    if (!loaders[tab] || !can(tabPermissions[tab])) return;
    const version = ++requestVersion.current;
    setBusy(true); setNotice("");
    try { const result = await loaders[tab](); if (version === requestVersion.current) { setData(rows(result)); setFailed(false); } }
    catch (error) { if (version === requestVersion.current) { setNotice(err(error, t.error)); setFailed(true); } }
    finally { if (version === requestVersion.current) setBusy(false); }
  }, [loaders, tab, t.error, can]);

  useEffect(() => { loadMeta().catch((error) => { setNotice(err(error, t.error)); setFailed(true); }); }, [loadMeta, t.error]);
  useEffect(() => { setData([]); reload(); return () => { requestVersion.current += 1; }; }, [reload]);
  ...
  const allowedTabs = tabs.map(([id])=>id).join('|');
  useEffect(() => { setTab(permittedTab(tabFromUrl(pageParams))); }, [pageParams,allowedTabs]);

  return <div className="operations-center">
    <header className="ops-hero"><div><p>{tx("AIMS OPERATIONS")}</p><h1>{t.title}</h1><span>{t.sub}</span></div><button className="secondary" onClick={reload} disabled={busy}><RefreshCw size={16} /> {t.refresh}</button></header>
    <nav className="ops-tabs">{tabs.map(([id, label, Icon]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => {setTab(id);setPageParams({tab:id})}}><Icon size={17} />{label}</button>)}</nav>
    <Notice value={notice} error={failed} />
    {tab === "landed" && data.some((item) => item.status === "posted") && <section className="ops-panel"><div className="ops-actions"><select aria-label={t.reverseLanded} value={landedReverseId} onChange={(event) => setLandedReverseId(event.target.value)}><option value="">{t.reverseLanded}</option>{data.filter((item) => item.status === "posted").map((item) => <option key={item.id} value={item.id}>{item.reference_number || `#${item.id}`} · {item.amount} {item.currency}</option>)}</select><button type="button" className="danger" disabled={busy || !landedReverseId} onClick={() => { const reason = window.prompt(t.reverseReason); if (reason?.trim()) done(() => reverseLandedCost(Number(landedReverseId), reason.trim())).then(() => setLandedReverseId("")); }}>{t.reverseLanded}</button></div></section>}
...
function Locator({ data, meta, run, t, can }) {
 const tx = useUiText()
  ...
  </form></aside>}<div className="ops-panel"><div className="ops-panel-head"><h2>{t.locator}</h2><div className="ops-filter-row"><select aria-label={tx("Filter warehouse")} value={warehouseFilter} onChange={(e) => setWarehouseFilter(e.target.value)}><option value="">{tx("All warehouses")}</option>{meta.warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select><input type="search" placeholder={tx("Search product, SKU, warehouse or bin…")} value={search} onChange={(e) => setSearch(e.target.value)} /></div></div><div className="ops-table"><table><thead><tr><th>{tx("Product")}</th><th>{tx("Warehouse")}</th><th>{tx("Bin")}</th><th>{tx("Available")}</th><th>{tx("Reserved")}</th><th>{tx("Damaged")}</th><th>{tx("Total")}</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td><strong>{item.product?.name}</strong><small>{item.product?.sku}</small></td><td>{item.warehouse?.name}</td><td>{item.location?.path || "Unassigned"}</td><td>{item.available_quantity}</td><td>{item.reserved_quantity}</td><td>{item.damaged_quantity}</td><td><strong>{item.quantity}</strong> {item.product?.unit}</td></tr>)}</tbody></table>{!filtered.length && <p>{t.empty}</p>}</div></div></section>;
}
  ...
  return <section className="ops-layout">
    ...
    <div className="ops-panel">
      ...
      {selected ? <>
        ...
        <div className="ops-table"><table><thead><tr><th>{tx("Recount")}</th><th>{tx("Product")}</th><th>{tx("Bin / state")}</th><th>{tx("Trace")}</th><th>{tx("Expected")}</th><th>{tx("Counted")}</th><th>{tx("Variance")}</th></tr></thead><tbody>{(selected.items || []).map((item) => <tr key={item.id}>
          <td><input aria-label={tx("Select for recount")} type="checkbox" disabled={["approved", "cancelled"].includes(selected.status)} checked={recountItems.includes(item.id)} onChange={() => setRecountItems(recountItems.includes(item.id) ? recountItems.filter((id) => id !== item.id) : [...recountItems, item.id])} /></td>
        ...
        <Field label={label(t, "Action reason", "Arsyeja e veprimit")}><input value={reason} onChange={(event) => setReason(event.target.value)} placeholder={tx("Required for recount, approval or cancellation")} /></Field>
...
function ExpiryQueue({ data, meta, t }) {
 const tx = useUiText()
  ...
  return <section className="ops-panel"><div className="ops-panel-head"><div><h2>{t.expiry}</h2><p>{tx("Live work queue for expired and near-expiry batches that still have stock.")}</p></div><div className="ops-filter-row"><select value={warehouse} onChange={(event) => setWarehouse(event.target.value)}><option value="">{tx("All warehouses")}</option>{meta.warehouses.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={product} onChange={(event) => setProduct(event.target.value)}><option value="">{tx("All expiry-controlled products")}</option>{expiryProducts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">{tx("All warning states")}</option><option value="expired">{tx("Expired")}</option><option value="near_expiry">{tx("Near expiry")}</option></select></div></div><div className="ops-table"><table><thead><tr><th>{tx("Status")}</th><th>{tx("Product")}</th><th>{tx("Lot / supplier batch")}</th><th>{tx("Expiry")}</th><th>{tx("Warehouse / bin")}</th><th>{tx("Quantity")}</th></tr></thead><tbody>{rows.map(({ lot, balance }) => <tr key={`${lot.id}-${balance.id}`} className={lot.expiry_status === "expired" ? "ops-expired" : "ops-near-expiry"}><td><strong>{lot.expiry_status}</strong></td><td>{lot.product?.name}<small>{lot.product?.sku}</small></td><td>{lot.lot_number || "—"}<small>{lot.supplier_batch || "—"}</small></td><td>{String(lot.expiry_at || "").slice(0, 10)}</td><td>{balance.warehouse?.name}<small>{balance.location?.path || "Unassigned"}</small></td><td>{balance.quantity}</td></tr>)}</tbody></table>{!rows.length && <p>{t.empty}</p>}</div></section>;
}
...
function Replenishment({ data, meta, run, t, can }) {
 const tx = useUiText()
  ...
  return <section className="ops-panel"><div className="ops-panel-head"><div><h2>{t.replenish}</h2><p>{tx("Every suggestion shows the stock, usage, lead-time, safety, MOQ and pack calculation used by AIMS.")}</p></div><div className="ops-actions"><select value={warehouse} onChange={(e) => setWarehouse(e.target.value)}><option value="">{tx("Default warehouse")}</option>{meta.warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select>{can("replenishment.manage") && <button disabled={!selected.length} onClick={() => run(() => createReplenishmentDrafts({ product_ids: selected, warehouse_id: warehouse ? +warehouse : null, notes: "Created from reviewed replenishment suggestions" }))}>{tx("Create draft PO")}</button>}</div></div><div className="ops-table ops-wide-table"><table><thead><tr><th></th><th>{tx("Product")}</th><th>{tx("On hand / reserved")}</th><th>{tx("Available + incoming")}</th><th>{tx("Projected / reorder point")}</th><th>{tx("Usage/day · history")}</th><th>{tx("Safety / minimum")}</th><th>{tx("Lead · stockout")}</th><th>{tx("MOQ / pack")}</th><th>{tx("Suggested")}</th><th>{tx("Supplier")}</th><th>{tx("Why")}</th></tr></thead><tbody>{data.map((item) => { const c = item.calculation || {}; const supplier = item.preferred_supplier || {}; return <tr key={item.product_id}><td>{can("replenishment.manage") && <input type="checkbox" checked={selected.includes(item.product_id)} onChange={() => setSelected(selected.includes(item.product_id) ? selected.filter((id) => id !== item.product_id) : [...selected, item.product_id])} />}</td><td><strong>{item.product_name}</strong><small>{item.sku} · {item.unit}</small></td><td>{c.on_hand} / {c.reserved}</td><td>{c.available} + {c.confirmed_incoming}</td><td><strong>{c.projected}</strong> / {c.effective_reorder_point}</td><td>{c.average_daily_usage}<small>{c.usage_in_history} {tx("in")} {c.history_days} {tx("days")}</small></td><td>{c.safety_stock} / {c.minimum_stock}</td><td>{c.supplier_lead_time_days} {tx("days")}<small>{item.expected_stockout_date || "No calculated stockout"}</small></td><td>{c.minimum_order_quantity} / {c.pack_size}</td><td><strong>{item.recommended_quantity}</strong></td><td>{supplier.supplier_name || "—"}<small>{supplier.purchase_price ?? "—"} {supplier.currency || ""}</small></td><td>{item.reason}<details><summary>{tx("Formula")}</summary><small>{(c.formula || []).join(" · ")}</small></details></td></tr>; })}</tbody></table>{!data.length && <p>{t.empty}</p>}</div></section>;
}
...
function Catalogue({ data, meta, run, t, can, baseCurrency }) {
 const tx = useUiText()
  ...
  const cards = <div className="ops-card-list">{data.map((item) => <article className={`ops-card ${item.is_active ? "" : "is-muted"}`} key={item.id}><div><strong>{item.product?.name}{item.is_preferred ? " ★" : ""}</strong><span>{item.supplier?.name} · {item.supplier_sku || "No supplier SKU"}</span>{item.supplier_description && <small>{item.supplier_description}</small>}</div><div><strong>{item.purchase_price ?? "—"} {item.currency}</strong><span>{tx("Pack")} {item.pack_size} {tx("· MOQ")} {item.minimum_order_quantity} · {item.usual_lead_time_days} {tx("days ·")} {item.is_active ? tx("Active") : tx("Inactive")}</span></div><div className="ops-actions"><button className="secondary" onClick={async () => { setHistory({ item, rows: await getProductSupplierHistory(item.id) }); }}>{tx("Price history")}</button><button className="secondary" onClick={async () => setPerformance(await getSupplierPerformance(item.supplier_id))}>{tx("Performance")}</button>{can("supplier_catalogue.manage") && <button className="secondary" onClick={() => edit(item)}>{tx("Edit")}</button>}{can("supplier_catalogue.manage") && item.is_active && <button className="danger" onClick={() => run(() => deactivateProductSupplier(item.id))}>{tx("Deactivate")}</button>}</div></article>)}</div>;
  ...
  return <section className="ops-layout"><aside className="ops-panel"><h2>{editing ? tx("Edit supplier offer") : tx("Add supplier offer")}</h2><form onSubmit={submit}>
    <Field label={tx("Product")}><select required disabled={Boolean(editing)} value={form.product_id} onChange={(e) => setForm({ ...form, product_id: e.target.value })}><option value="">—</option>{meta.products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
    <Field label={tx("Supplier")}><select required disabled={Boolean(editing)} value={form.supplier_id} onChange={(e) => setForm({ ...form, supplier_id: e.target.value })}><option value="">—</option>{meta.suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field>
    <Field label={tx("Supplier SKU")}><input value={form.supplier_sku} onChange={(e) => setForm({ ...form, supplier_sku: e.target.value })} /></Field>
    <div className="ops-form-grid"><Field label={tx("Price")}><input type="number" min="0" step="0.000001" value={form.purchase_price} onChange={(e) => setForm({ ...form, purchase_price: e.target.value })} /></Field><Field label={tx("Currency")}><select value={form.currency} onChange={(e) => setForm({ ...form, currency: e.target.value, exchange_rate_to_base: e.target.value === baseCurrency ? 1 : form.exchange_rate_to_base })}>{[...new Set([baseCurrency, "EUR", "USD", "ALL", "GBP", "CHF", "CNY"])].map((currency) => <option key={currency}>{currency}</option>)}</select></Field>{form.currency !== baseCurrency && <><Field label={`Rate to ${baseCurrency}`}><input required type="number" min="0.00000001" step="0.00000001" value={form.exchange_rate_to_base} onChange={(e) => setForm({ ...form, exchange_rate_to_base: e.target.value })} /></Field><Field label={tx("Rate date")}><input required type="date" value={form.exchange_rate_date} onChange={(e) => setForm({ ...form, exchange_rate_date: e.target.value })} /></Field></>}<Field label={tx("Pack size")}><input type="number" min="0.001" step="0.001" value={form.pack_size} onChange={(e) => setForm({ ...form, pack_size: e.target.value })} /></Field><Field label={tx("MOQ")}><input type="number" min="0" step="0.001" value={form.minimum_order_quantity} onChange={(e) => setForm({ ...form, minimum_order_quantity: e.target.value })} /></Field><Field label={tx("Lead days")}><input type="number" min="0" value={form.usual_lead_time_days} onChange={(e) => setForm({ ...form, usual_lead_time_days: e.target.value })} /></Field></div>
    <Field label={tx("Supplier product description")}><textarea value={form.supplier_description} onChange={(e) => setForm({ ...form, supplier_description: e.target.value })} /></Field>
    <div className="ops-form-grid"><Field label={tx("Price effective date")}><input type="date" value={form.price_effective_at} onChange={(e) => setForm({ ...form, price_effective_at: e.target.value })} /></Field><Field label={tx("Price change reason")}><input value={form.price_change_reason} onChange={(e) => setForm({ ...form, price_change_reason: e.target.value })} /></Field></div>
    <label className="ops-check"><input type="checkbox" checked={form.is_preferred} onChange={(e) => setForm({ ...form, is_preferred: e.target.checked })} /> {tx("Preferred supplier")}</label><label className="ops-check"><input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} /> {tx("Active")}</label>
    <div className="ops-actions"><button>{t.save}</button>{editing && <button type="button" className="secondary" onClick={() => { setEditing(null); setForm(blank()); }}>{tx("Cancel edit")}</button>}</div>
...
function CatalogueInsights({ performance, history, baseCurrency }) {
 const tx = useUiText()
  ...
  return <>{history && <section className="ops-insight"><h3>{history.item.product?.name} {tx("· price history")}</h3><div className="ops-table"><table><thead><tr><th>{tx("Effective")}</th><th>{tx("Price")}</th><th>{baseCurrency} {tx("value")}</th><th>{tx("Changed by")}</th><th>{tx("Reason")}</th></tr></thead><tbody>{history.rows.map((row) => <tr key={row.id}><td>{String(row.effective_at || "").slice(0, 10)}</td><td>{row.purchase_price} {row.currency}</td><td>{row.base_currency_price}</td><td>{row.changed_by?.name || "—"}</td><td>{row.change_reason || "—"}</td></tr>)}</tbody></table></div></section>}{performance && <section className="ops-insight"><h3>{tx("Authoritative supplier scorecard ·")} {performance.supplier_name}</h3><p>{tx("One shared score is used here, in Quality Management, Suppliers and Procurement.")}</p><div className="ops-metric-grid"><div><span>{tx("Overall score")}</span><strong>{performance.overall_score ?? "Insufficient data"}</strong></div>{Object.entries(categories).map(([key, value]) => <div key={key}><span>{key.toLowerCase()}</span><strong>{value?.score ?? "—"}</strong></div>)}</div>{performance.explanation?.map((line) => <small key={line}>{line}</small>)}</section>}</>;
}
...
function LandedCosts({ data, meta, run, t, can, baseCurrency }) {
 const tx = useUiText()
  ...
  return <section className="ops-layout"><aside className="ops-panel"><h2>{tx("Allocate import cost")}</h2><form onSubmit={submit}><Field label={tx("Goods receipt")}><select required value={form.goods_receipt_id} onChange={(e) => setForm({ ...form, goods_receipt_id: e.target.value })}><option value="">{meta.receipts.length ? tx("Select a goods receipt") : tx("No received purchase orders available")}</option>{meta.receipts.map((r) => <option key={r.id} value={r.id}>{r.receipt_number} · {r.purchase_order?.po_number || ""}</option>)}</select>{!meta.receipts.length && <small className="ops-field-hint">{tx("Receive products from a purchase order first, then allocate freight, customs or other import costs here.")}</small>}</Field><Field label={tx("Cost type")}><select value={form.cost_type} onChange={(e) => setForm({ ...form, cost_type: e.target.value })}>{["freight", "customs", "insurance", "port", "forwarding", "inland_transport", "handling", "other"].map((v) => <option key={v}>{v.replaceAll("_", " ")}</option>)}</select></Field><div className="ops-form-grid"><Field label={tx("Amount")}><input required type="number" min="0.01" step="0.01" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} /></Field><Field label={tx("Currency")}><select value={form.currency} onChange={(e) => setForm({ ...form, currency: e.target.value, exchange_rate_to_base: e.target.value === baseCurrency ? 1 : form.exchange_rate_to_base })}>{[...new Set([baseCurrency, "EUR", "USD", "ALL", "GBP", "CHF", "CNY"])].map((currency) => <option key={currency}>{currency}</option>)}</select></Field>{form.currency !== baseCurrency && <><Field label={`Rate to ${baseCurrency}`}><input required type="number" min="0.00000001" step="0.00000001" value={form.exchange_rate_to_base} onChange={(e) => setForm({ ...form, exchange_rate_to_base: e.target.value })} /></Field><Field label={tx("Rate date")}><input required type="date" value={form.exchange_rate_date} onChange={(e) => setForm({ ...form, exchange_rate_date: e.target.value })} /></Field></>}<Field label={tx("Allocation")}><select value={form.allocation_method} onChange={(e) => setForm({ ...form, allocation_method: e.target.value })}>{["quantity", "value", "weight", "volume", "manual"].map((v) => <option key={v}>{v}</option>)}</select></Field></div>{receiptBusy && <small>{tx("Loading receipt lines…")}</small>}{receiptItems.length > 0 && <fieldset className="ops-allocation-lines"><legend>{label(t, "Receipt lines", "Rreshtat e pranimit")}</legend>{receiptItems.map((item) => { const quantity = +(item.accepted_base_quantity ?? item.accepted_quantity ?? 0) + +(item.damaged_base_quantity ?? item.damaged_quantity ?? 0); return <label key={item.id} className="ops-allocation-line"><input type="checkbox" checked={selectedItems.includes(item.id)} onChange={(e) => setSelectedItems(e.target.checked ? [...selectedItems, item.id] : selectedItems.filter((id) => id !== item.id))} /><span><strong>{item.product?.name || item.purchase_order_item?.description}</strong><small>{quantity} {item.product?.unit || item.purchase_order_item?.unit}</small></span>{form.allocation_method === "manual" && selectedItems.includes(item.id) && <input aria-label={`Amount for ${item.product?.name || item.id}`} required type="number" min="0.01" step="0.01" value={manualAmounts[item.id] || ""} onChange={(e) => setManualAmounts({ ...manualAmounts, [item.id]: e.target.value })} />}</label>; })}{form.allocation_method === "manual" && <p className={!manualComplete ? "ops-total is-invalid" : "ops-total"}>{tx("Allocated:")} {manualTotal.toFixed(2)} / {(+form.amount || 0).toFixed(2)} {form.currency}</p>}</fieldset>}<Field label={tx("Reference")}><input value={form.reference_number} onChange={(e) => setForm({ ...form, reference_number: e.target.value })} /></Field><Field label={tx("Description")}><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></Field><button disabled={!selectedItems.length || (form.allocation_method === "manual" && !manualComplete)}>{t.create}</button></form></aside><div className="ops-panel"><h2>{t.landed}</h2><div className="ops-card-list">{data.map((item) => <article className="ops-card" key={item.id}><div><strong>{(item.cost_type || "other").replaceAll("_", " ")} · {item.base_currency_amount} {item.base_currency || baseCurrency}</strong><span>{item.goods_receipt?.receipt_number} · {item.amount} {item.currency} @ {item.exchange_rate_to_base} · {item.allocation_method} · {item.status}</span>{item.status === "posted" && <small>{tx("Inventory / Inventari:")} {Number(item.inventory_adjustment_total || 0).toFixed(2)} {item.base_currency || baseCurrency} {tx("· COGS:")} {Number(item.cogs_adjustment_total || 0).toFixed(2)} {item.base_currency || baseCurrency}</small>}</div>{can("landed_costs.manage") && item.status === "draft" && <div className="ops-actions"><button onClick={() => run(() => postLandedCost(item.id))}>{tx("Post allocation")}</button><button className="danger" onClick={() => { if (window.confirm("Delete this draft landed cost?")) run(() => deleteLandedCost(item.id)); }}>{tx("Delete draft")}</button></div>}</article>)}</div></div></section>;
}
...
function Returns({ data, meta, run, t, can, baseCurrency }) {
 const tx = useUiText()
  ...
  useEffect(() => {
    const id = Number(documentParams.get('return'));
    if (id) open(id);
  }, [documentParams]);
  const idempotencyKeyRef = useRef(uuid());
  ...
  return <section className={`ops-layout ${can("returns.manage") ? "" : "is-single"}`}>
    {can("returns.manage") && <aside className="ops-panel"><h2>{editing ? `Edit ${selected?.return_number}` : tx("New controlled return")}</h2><form onSubmit={submit}>
      <Field label={tx("Return type")}><select value={form.type} onChange={(event) => setForm({ ...blankForm(), type: event.target.value })}><option value="customer">{tx("Customer return")}</option><option value="supplier">{tx("Supplier return")}</option></select></Field>
      {form.type === "customer" ? <Field label={tx("Customer")}><select required value={form.customer_id} onChange={(event) => setForm({ ...form, customer_id: event.target.value, source_id: "" })}><option value="">—</option>{meta.customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></Field> : <Field label={tx("Supplier")}><select required value={form.supplier_id} onChange={(event) => setForm({ ...form, supplier_id: event.target.value, source_id: "" })}><option value="">—</option>{meta.suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></Field>}
      <Field label={form.type === "customer" ? tx("Original daily sale (optional)") : tx("Original goods receipt (optional)")}><select value={form.source_id} onChange={(event) => setForm({ ...form, source_id: event.target.value, items: [blankItem()] })}><option value="">{tx("No source document")}</option>{sourceRecords.map((record) => <option key={record.id} value={record.id}>{record.sale_number || record.receipt_number} · {String(record.sale_date || record.received_at || "").slice(0, 10)}</option>)}</select></Field>
      <fieldset className="ops-lines"><legend>{label(t, "Return lines", "Artikujt e kthimit")}</legend>{form.items.map((line, index) => { const product = meta.products.find((item) => String(item.id) === String(line.product_id)); const mode = product?.tracking_mode || "none"; const lineLocations = meta.locations.filter((location) => String(location.warehouse_id) === String(line.warehouse_id)); return <div className="ops-return-line" key={index}>
        {source ? <Field label={tx("Source line")}><select required value={line.source_item_id} onChange={(event) => { const sourceLine = sourceItems.find((item) => String(item.id) === event.target.value); updateLine(index, { source_item_id: event.target.value, product_id: String(sourceLine?.product_id || ""), quantity: sourceLine?.base_quantity || (+sourceLine?.accepted_base_quantity + +sourceLine?.damaged_base_quantity) || "", warehouse_id: String(source.warehouse_id || line.warehouse_id), location_id: source.location_id ? String(source.location_id) : "" }); }}><option value="">—</option>{sourceItems.map((item) => <option key={item.id} value={item.id}>{item.product?.name} · {item.base_quantity || (+item.accepted_base_quantity + +item.damaged_base_quantity)} {item.product?.unit}</option>)}</select></Field> : <Field label={tx("Product")}><select required value={line.product_id} onChange={(event) => updateLine(index, { product_id: event.target.value, lot_number: "", serial_numbers: "", expiry_at: "" })}><option value="">—</option>{meta.products.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>}
        <div className="ops-form-grid"><Field label={tx("Warehouse")}><select required value={line.warehouse_id} onChange={(event) => updateLine(index, { warehouse_id: event.target.value, location_id: "" })}><option value="">—</option>{meta.warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></Field><Field label={tx("Bin")}><select value={line.location_id} onChange={(event) => updateLine(index, { location_id: event.target.value })}><option value="">{tx("Unassigned")}</option>{lineLocations.map((location) => <option key={location.id} value={location.id}>{location.path || location.name}</option>)}</select></Field><Field label={tx("Condition")}><select value={line.condition} onChange={(event) => updateLine(index, { condition: event.target.value })}>{["sellable", "damaged", "quarantine", "scrap"].map((value) => <option key={value}>{value}</option>)}</select></Field></div>
        {mode === "serial" ? <Field label={`Serial numbers (${serials(line).length})`}><textarea required value={line.serial_numbers} onChange={(event) => updateLine(index, { serial_numbers: event.target.value })} placeholder={tx("One serial per line")} /></Field> : <Field label={tx("Quantity")}><input required type="number" min="0.001" step="0.001" value={line.quantity} onChange={(event) => updateLine(index, { quantity: event.target.value })} /></Field>}
        {["batch", "batch_expiry"].includes(mode) && <div className="ops-form-grid"><Field label={tx("Lot / batch")}><input required value={line.lot_number} onChange={(event) => updateLine(index, { lot_number: event.target.value })} /></Field><Field label={tx("Supplier batch")}><input value={line.supplier_batch} onChange={(event) => updateLine(index, { supplier_batch: event.target.value })} /></Field><Field label={tx("Manufactured")}><input type="date" value={line.manufactured_at} onChange={(event) => updateLine(index, { manufactured_at: event.target.value })} /></Field><Field label={tx("Expiry")}><input required={mode === "batch_expiry"} type="date" value={line.expiry_at} onChange={(event) => updateLine(index, { expiry_at: event.target.value })} /></Field></div>}
        <Field label={tx("Line notes")}><input value={line.notes} onChange={(event) => updateLine(index, { notes: event.target.value })} /></Field><button type="button" className="danger" disabled={form.items.length === 1} onClick={() => setForm({ ...form, items: form.items.filter((_, itemIndex) => itemIndex !== index) })}>{tx("Remove line")}</button>
      </div>; })}<button type="button" className="secondary" onClick={() => setForm({ ...form, items: [...form.items, blankItem()] })}>{tx("+ Add return line")}</button></fieldset>
      <Field label={tx("Financial resolution")}><select value={form.financial_resolution} onChange={(event) => setForm({ ...form, financial_resolution: event.target.value })}>{(form.type === "customer" ? ["none", "cash_refund", "debt_credit"] : ["none", "cash_refund", "supplier_credit", "replacement"]).map((value) => <option key={value}>{value}</option>)}</select></Field>
      {!["none", "replacement"].includes(form.financial_resolution) && <Field label={tx("Amount")}><input required type="number" min="0.01" step="0.01" value={form.financial_amount} onChange={(event) => setForm({ ...form, financial_amount: event.target.value })} /></Field>}
      {form.financial_resolution === "cash_refund" && <Field label={tx("Money account")}><select required value={form.financial_account_id} onChange={(event) => setForm({ ...form, financial_account_id: event.target.value })}><option value="">—</option>{meta.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Field>}
      <Field label={form.financial_resolution === "replacement" ? tx("Replacement reference / status note") : tx("Reference number")}><input value={form.reference_number} onChange={(event) => setForm({ ...form, reference_number: event.target.value })} /></Field><Field label={tx("Reason")}><textarea required minLength="3" value={form.reason} onChange={(event) => setForm({ ...form, reason: event.target.value })} /></Field><Field label={tx("Notes")}><textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></Field><div className="ops-actions"><button>{editing ? t.save : t.create}</button>{editing && <button type="button" className="secondary" onClick={() => { setEditing(false); setForm(blankForm()); }}>{tx("Cancel edit")}</button>}</div>
    </form></aside>}
    <div className="ops-panel"><h2>{t.returns}</h2><div className="ops-card-list">{data.map((item) => <article className="ops-card" key={item.id}><div><strong>{item.return_number} · {item.type}</strong><span>{item.customer?.name || item.supplier?.name} · {item.status} · {item.items?.length || 0} {tx("lines")}</span></div><button className="secondary" onClick={() => open(item.id)}>{tx("View")}</button></article>)}</div>
      {selected && <section className="ops-detail"><EntityDocuments entityType="return" entityId={selected.id}/><h3>{selected.return_number} · {selected.status}</h3><p>{selected.reason}</p><div className="ops-table"><table><thead><tr><th>{tx("Product")}</th><th>{tx("Source")}</th><th>{tx("Destination")}</th><th>{tx("Quantity")}</th><th>{tx("Condition")}</th><th>{tx("Processed")}</th></tr></thead><tbody>{selected.items.map((item) => <tr key={item.id}><td>{item.product?.name}</td><td>{item.daily_sale_item_id ? `Sale line ${item.daily_sale_item_id}` : item.goods_receipt_item_id ? `Receipt line ${item.goods_receipt_item_id}` : tx("Manual")}</td><td>{item.warehouse?.name} · {item.location?.path || "Unassigned"}</td><td>{item.quantity} {item.unit}</td><td>{item.condition}</td><td>{item.processed_quantity}</td></tr>)}</tbody></table></div><Field label={tx("Action/cancellation reason")}><input value={actionReason} onChange={(event) => setActionReason(event.target.value)} /></Field><div className="ops-actions">{can("returns.manage") && selected.status === "draft" && <><button onClick={edit}>{tx("Edit")}</button><button onClick={() => act("submit")}>{tx("Submit")}</button><button className="danger" onClick={() => run(async () => { await deleteInventoryReturn(selected.id); setSelected(null); })}>{tx("Delete draft")}</button></>}{can("returns.approve") && selected.status === "submitted" && <button onClick={() => act("approve")}>{tx("Approve")}</button>}{can("returns.approve") && selected.status === "approved" && <button onClick={() => act("complete")}>{tx("Complete")}</button>}{can("returns.manage") && !["completed", "cancelled"].includes(selected.status) && <button className="danger" disabled={actionReason.trim().length < 3} onClick={() => act("cancel", { reason: actionReason })}>{tx("Cancel return")}</button>}</div><details className="ops-history"><summary>{label(t, "Audit history", "Historia e auditimit")}</summary>{(selected.events || []).map((event) => <article key={event.id}><strong>{event.action}</strong><span>{event.created_at ? new Date(event.created_at).toLocaleString() : "—"}</span>{event.reason && <p>{event.reason}</p>}</article>)}</details></section>}
    </div>
  </section>;
}
...
function SupplierMatching({ data, meta, run, t, can }) {
 const tx = useUiText()
  ...
  const records = <div className="ops-card-list">{data.map((item) => <article className="ops-card" key={item.id}><div><strong>{item.document_number} · {item.vendor_name}</strong><span>{item.purchase_order?.po_number || "No PO"} · {item.match_status} · {item.gross_amount} {item.currency}</span></div><button className="secondary" onClick={() => open(item.id)}>{tx("Review match")}</button></article>)}</div>;
  ...
  return <section className={`ops-layout ${can("supplier_invoices.manage") ? "" : "is-single"}`}>{can("supplier_invoices.manage") && <aside className="ops-panel"><h2>{editing ? `Edit ${selected?.document_number}` : tx("New supplier invoice")}</h2><form onSubmit={submit}><Field label={tx("Supplier")}><select required value={form.supplier_id} onChange={(e) => setForm({ ...form, supplier_id: e.target.value })}><option value="">—</option>{meta.suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></Field><Field label={tx("Purchase Order (optional)")}><select value={form.purchase_order_id} onChange={(e) => chooseOrder(e.target.value)}><option value="">{tx("No PO")}</option>{meta.orders.map((po) => <option key={po.id} value={po.id}>{po.po_number}</option>)}</select></Field>{orderReceipts.length > 0 && <fieldset className="ops-check-group"><legend>{tx("Goods receipts included in this match")}</legend>{orderReceipts.map((receipt) => <label key={receipt.id}><input type="checkbox" checked={selectedReceiptIds.includes(receipt.id)} onChange={(event) => setSelectedReceiptIds(event.target.checked ? [...selectedReceiptIds, receipt.id] : selectedReceiptIds.filter((id) => id !== receipt.id))} /> {receipt.receipt_number} · {String(receipt.received_at).slice(0, 10)}</label>)}</fieldset>}<div className="ops-form-grid"><Field label={tx("Document number")}><input required value={form.document_number} onChange={(e) => setForm({ ...form, document_number: e.target.value })} /></Field><Field label={tx("Invoice date")}><input type="date" required value={form.invoice_date} onChange={(e) => setForm({ ...form, invoice_date: e.target.value })} /></Field></div><div className="ops-form-grid"><Field label={label(t, "VAT rate", "Norma e TVSH-së")}><select value={form.vat_rate} onChange={(e) => { const rate = +e.target.value; setForm({ ...form, vat_rate: rate, input_vat_eligible: rate > 0 ? form.input_vat_eligible : false, lines: form.lines.map((line) => ({ ...line, vat_rate: rate })) }); }}><option value="0">{tx("0% · No VAT")}</option><option value="8">{tx("8% · Reduced")}</option><option value="18">{tx("18% · Standard")}</option></select></Field><label className="ops-check"><input type="checkbox" disabled={!+form.vat_rate} checked={form.input_vat_eligible} onChange={(e) => setForm({ ...form, input_vat_eligible: e.target.checked })} /> {tx("VAT deductible")}</label></div><fieldset className="ops-lines"><legend>{tx("Invoice lines")}</legend>{form.lines.map((line, index) => <div className="ops-invoice-line" key={index}><Field label={tx("Product")}><select required value={line.product_id} onChange={(e) => { const product = meta.products.find((item) => String(item.id) === e.target.value); updateLine(index, { product_id: e.target.value, description: product?.name || line.description, unit: product?.unit || line.unit }); }}><option value="">—</option>{meta.products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></Field>{line.purchase_order_item_id && <Field label={tx("PO line")}><select value={line.purchase_order_item_id} onChange={(e) => updateLine(index, { purchase_order_item_id: e.target.value, goods_receipt_item_id: "" })}><option value="">—</option>{(order?.items || []).map((item) => <option key={item.id} value={item.id}>{item.description} {tx("· ordered")} {item.quantity}</option>)}</select></Field>}<Field label={tx("Receipt line")}><select value={line.goods_receipt_item_id || ""} onChange={(e) => updateLine(index, { goods_receipt_item_id: e.target.value })}><option value="">{tx("No receipt line")}</option>{selectableReceiptItems.filter((item) => !line.purchase_order_item_id || +item.purchase_order_item_id === +line.purchase_order_item_id).map((item) => <option key={item.id} value={item.id}>{item.receipt.receipt_number} · {item.product?.name} {tx("· received")} {+item.accepted_quantity + +item.damaged_quantity}</option>)}</select></Field><div className="ops-form-grid"><Field label={tx("Invoiced quantity")}><input type="number" min="0.001" step="0.001" value={line.quantity} onChange={(e) => updateLine(index, { quantity: e.target.value })} /></Field><Field label={tx("Invoice unit price")}><input type="number" min="0" step="0.0001" value={line.unit_price} onChange={(e) => updateLine(index, { unit_price: e.target.value })} /></Field></div><button type="button" className="danger" disabled={form.lines.length === 1} onClick={() => setForm({ ...form, lines: form.lines.filter((_, lineIndex) => lineIndex !== index) })}>{tx("Remove")}</button></div>)}<button type="button" className="secondary" onClick={() => setForm({ ...form, lines: [...form.lines, blankLine(form.vat_rate)] })}>{tx("+ Add line")}</button></fieldset><div className="ops-actions"><button>{editing ? t.save : t.create}</button>{editing && <button type="button" className="secondary" onClick={() => setEditing(null)}>{tx("Cancel edit")}</button>}</div></form></aside>}<div className="ops-panel"><h2>{t.matching}</h2>{records}{selected && <section className="ops-detail"><h3>{selected.document_number} · {selected.match_status}</h3><p>{tx("PO")} {selected.purchase_order?.po_number || "—"} {tx("· Invoice")} {selected.gross_amount} {selected.currency}</p>{selected.purchase_order_id && can("supplier_invoices.manage") && <section className="ops-payment-allocation"><h4>{t.allocatePayment}</h4>{paymentOptions.length ? <form onSubmit={submitAllocation}><Field label={t.payment}><select required value={allocation.purchase_order_payment_id} onChange={(event) => { const payment = paymentOptions.find((row) => String(row.id) === event.target.value); const suggested = Math.min(Number(payment?.available || 0), Number(selected.remaining_amount || 0)); setAllocation({ ...allocation, purchase_order_payment_id: event.target.value, amount: suggested > 0 ? suggested.toFixed(2) : "" }); }}><option value="">—</option>{paymentOptions.map((payment) => <option key={payment.id} value={payment.id}>{String(payment.payment_date || "").slice(0, 10)} · {payment.reference_number || `#${payment.id}`} · {payment.available.toFixed(2)} {payment.currency || selected.currency} {t.availablePayment}</option>)}</select></Field><div className="ops-form-grid"><Field label={t.paymentAmount}><input required type="number" min="0.01" max={allocationMaximum || undefined} step="0.01" value={allocation.amount} onChange={(event) => setAllocation({ ...allocation, amount: event.target.value })} /></Field><Field label={t.allocationReason}><input value={allocation.reason} onChange={(event) => setAllocation({ ...allocation, reason: event.target.value })} /></Field></div><button disabled={!allocation.purchase_order_payment_id || !Number(allocation.amount) || Number(allocation.amount) > allocationMaximum + .001}>{t.allocate}</button></form> : <p className="ops-empty">{t.noAdvance}</p>}{(selected.purchase_order_payment_allocations || []).length > 0 && <div className="ops-allocation-history"><strong>{t.existingAllocations}</strong>{selected.purchase_order_payment_allocations.map((row) => <span key={row.id}>{String(row.allocated_at || row.created_at || "").slice(0, 10)} · {row.payment?.reference_number || `#${row.purchase_order_payment_id}`} · {row.amount} {selected.currency}</span>)}</div>}</section>}<div className="ops-table ops-wide-table"><table><thead><tr><th>{tx("Line")}</th><th>{tx("Ordered")}</th><th>{tx("Received")}</th><th>{tx("Previously invoiced")}</th><th>{tx("This invoice")}</th><th>{tx("Quantity variance")}</th><th>{tx("Price variance")}</th><th>{tx("Tax variance")}</th><th>{tx("Issues")}</th></tr></thead><tbody>{summaryLines.map((line) => { const variance = line.variance || {}; return <tr key={line.id}><td>{line.product?.name || line.description}</td><td>{variance.ordered ?? "—"}</td><td>{variance.received ?? "—"}</td><td>{variance.otherInvoiced ?? "—"}</td><td>{variance.invoiced ?? line.quantity}</td><td>{variance.quantityVariance ?? "—"}</td><td>{variance.priceVariance ?? "—"}</td><td>{variance.taxVariance ?? "—"}</td><td>{(variance.issues || []).join(", ") || "Matched"}</td></tr>; })}</tbody></table></div><p className="ops-summary">{tx("Tolerances: quantity")} {selected.match_summary?.tolerances?.quantity ?? "—"}{tx(", unit price")} {selected.match_summary?.tolerances?.unit_price ?? "—"}{tx(", tax")} {selected.match_summary?.tolerances?.tax ?? "—"}</p><Field label={tx("Exception approval reason")}><textarea value={approvalReason} onChange={(e) => setApprovalReason(e.target.value)} placeholder={tx("Required when approving an exception or unmatched invoice")} /></Field><div className="ops-actions">{can("supplier_invoices.manage") && selected.status === "draft" && !selected.approved_at && <button onClick={edit}>{tx("Edit draft")}</button>}{can("supplier_invoices.manage") && <button className="secondary" onClick={() => run(async () => setSelected(await recalculateSupplierInvoice(selected.id)))}>{tx("Recalculate")}</button>}{can("supplier_invoices.approve") && selected.match_status !== "approved" && <button disabled={["exception", "unmatched"].includes(selected.match_status) && approvalReason.trim().length < 3} onClick={() => run(async () => setSelected(await approveSupplierInvoice(selected.id, approvalReason || null)))}>{tx("Approve")}</button>}</div><details className="ops-history"><summary>{tx("Matching audit history")}</summary>{(selected.match_events || []).map((event) => <article key={event.id}><strong>{event.action}</strong><span>{event.created_at ? new Date(event.created_at).toLocaleString() : "—"}</span>{event.reason && <p>{event.reason}</p>}</article>)}</details></section>}</div></section>;
}
