...
function Catalogue({ data, meta, run, t, can }) {
  ...
  const blank = () => ({ product_id: "", supplier_id: "", supplier_sku: "", purchase_price: "", currency: "EUR", exchange_rate_to_base: 1, exchange_rate_date: today, pack_size: 1, minimum_order_quantity: 0, usual_lead_time_days: 0, is_preferred: false, is_active: true, supplier_description: "", price_effective_at: today, price_change_reason: "" });
  ...
  </form></aside><div className="ops-panel"><h2>{t.catalogue}</h2>{cards}<CatalogueInsights performance={performance} history={history} /></div></section>;
}
...
function LandedCosts({ data, meta, run, t, can }) {
  const [form, setForm] = useState({ goods_receipt_id: "", cost_type: "freight", amount: "", currency: "EUR", exchange_rate_to_base: 1, exchange_rate_date: new Date().toISOString().slice(0, 10), allocation_method: "value", reference_number: "", description: "" });
  ...
  const submit = (event) => {
    event.preventDefault();
    const payload = { ...form, goods_receipt_id: +form.goods_receipt_id, amount: +form.amount, exchange_rate_to_base: form.currency === "EUR" ? 1 : +form.exchange_rate_to_base, exchange_rate_date: form.currency === "EUR" ? null : form.exchange_rate_date, idempotency_key: uuid() };
