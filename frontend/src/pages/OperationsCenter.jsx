...
import {
  ...
  deactivateProductSupplier, deleteInventoryReturn, getExpiringInventory, getFinancialAccounts,
...
const text = {
  en: { title: "Inventory & Supply Control", sub: "Bins, counts, replenishment, supplier prices, landed cost, returns and supplier matching in one connected workspace.", locator: "Bin stock", counts: "Physical counts", expiry: "Expiry control", replenish: "Replenishment", catalogue: "Supplier catalogue", landed: "Landed cost", returns: "Returns", matching: "Supplier matching", save: "Save", create: "Create", refresh: "Refresh", all: "All", loading: "Loading…", empty: "No records found.", error: "The operation could not be completed.", success: "Saved successfully.", allocatePayment: "Allocate Purchase Order payment", payment: "Payment", paymentAmount: "Amount to allocate", allocationReason: "Allocation note", noAdvance: "No unallocated Purchase Order payment is available.", existingAllocations: "Allocated advances", availablePayment: "available", allocate: "Allocate" },
  sq: { title: "Kontrolli i Inventarit dhe Furnizimit", sub: "Lokacionet, numërimet, rimbushja, çmimet, kostoja e importit, kthimet dhe përputhja e furnizuesit në një hapësirë.", locator: "Stoku sipas lokacionit", counts: "Numërimi fizik", expiry: "Kontrolli i skadimit", replenish: "Rimbushja", catalogue: "Katalogu i furnizuesit", landed: "Kostoja e importit", returns: "Kthimet", matching: "Përputhja e furnizuesit", save: "Ruaj", create: "Krijo", refresh: "Rifresko", all: "Të gjitha", loading: "Duke ngarkuar…", empty: "Nuk ka të dhëna.", error: "Veprimi nuk mund të kryhej.", success: "U ruajt me sukses.", allocatePayment: "Aloko pagesën e porosisë", payment: "Pagesa", paymentAmount: "Shuma për alokim", allocationReason: "Shënim për alokimin", noAdvance: "Nuk ka pagesë të paalokuar të porosisë.", existingAllocations: "Avanset e alokuara", availablePayment: "në dispozicion", allocate: "Aloko" },
};
...
export default function OperationsCenter() {
  ...
  return <div className="operations-center">
    ...
    {tab === "catalogue" && <Catalogue data={data} meta={meta} run={done} t={t} can={can} />}
    {tab === "landed" && <LandedCosts data={data} meta={meta} run={done} t={t} can={can} />}
    {tab === "returns" && <Returns data={data} meta={meta} run={done} t={t} can={can} />}
...
function Locator({ data, meta, run, t, can }) {
  ...
  return <section className={`ops-layout ${can("stock.manage") ? "" : "is-single"}`}>{can("stock.manage") && <aside className="ops-panel"><h2>Move between bins</h2><form onSubmit={(event) => { event.preventDefault(); run(() => moveBinStock({ product_id: +form.product_id, source_location_id: +form.source_location_id, destination_location_id: +form.destination_location_id, quantity: +form.quantity, stock_state: form.stock_state, reason: form.reason, idempotency_key: uuid() })); }}>
    <Field label="Product / Produkti"><select required value={form.product_id} onChange={(e) => setForm({ ...form, product_id: e.target.value })}><option value="">—</option>{meta.products.map((p) => <option key={p.id} value={p.id}>{p.name} · {p.sku}</option>)}</select></Field>
    <Field label="Source warehouse / Magazina burim"><select required value={form.source_warehouse_id} onChange={(e) => setForm({ ...form, source_warehouse_id: e.target.value, source_location_id: "" })}><option value="">—</option>{meta.warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></Field>
    <Field label="Source bin / Lokacioni burim"><select required value={form.source_location_id} onChange={(e) => setForm({ ...form, source_location_id: e.target.value })}><option value="">—</option>{sourceLocations.map((l) => <option key={l.id} value={l.id}>{l.path || l.name}</option>)}</select></Field>
    ...
    <Field label="Destination bin / Destinacioni"><select required value={form.destination_location_id} onChange={(e) => setForm({ ...form, destination_location_id: e.target.value })}><option value="">—</option>{destinationLocations.map((l) => <option key={l.id} value={l.id}>{l.path || l.name}</option>)}</select></Field>
    <Field label="Quantity / Sasia"><input required type="number" min="0.001" step="0.001" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} /></Field><button>{t.save}</button>
  ...
  return <section className="ops-layout">
    <aside className="ops-panel">
      ...
      <form onSubmit={(event) => { event.preventDefault(); run(() => createInventoryCount({ warehouse_id: +form.warehouse_id, location_id: form.location_id ? +form.location_id : null, product_ids: form.product_ids.map(Number), stock_states: form.stock_states, notes: form.notes || null })); }}>
        <Field label="Warehouse / Magazina"><select required value={form.warehouse_id} onChange={(event) => setForm({ ...form, warehouse_id: event.target.value, location_id: "" })}><option value="">—</option>{meta.warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></Field>
        <Field label="Location scope / Lokacioni"><select value={form.location_id} onChange={(event) => setForm({ ...form, location_id: event.target.value })}><option value="">Full warehouse / E gjithë magazina</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.path || location.name}</option>)}</select></Field>
        <Field label="Products (optional) / Produktet"><select multiple size="5" value={form.product_ids.map(String)} onChange={(event) => setForm({ ...form, product_ids: [...event.target.selectedOptions].map((option) => option.value) })}>{meta.products.map((product) => <option key={product.id} value={product.id}>{product.name} · {product.sku || "—"}</option>)}</select></Field>
        <fieldset className="ops-check-group"><legend>Stock states / Gjendjet</legend>{["available", "damaged", "quarantine"].map((state) => <label key={state}><input type="checkbox" checked={form.stock_states.includes(state)} onChange={(event) => setState(state, event.target.checked)} /> {state}</label>)}</fieldset>
        <Field label="Notes / Shënime"><textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></Field>
    ...
    <div className="ops-panel">
      ...
      {selected ? <>
        <p className="ops-summary">Frozen: {selected.frozen_at ? new Date(selected.frozen_at).toLocaleString() : "—"} · Scope: {(selected.stock_states || []).join(", ")}</p>
        ...
        <Field label="Action reason / Arsyeja"><input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Required for recount, approval or cancellation" /></Field>
        ...
        <details className="ops-history"><summary>Count and recount history / Historia</summary>{(selected.items || []).flatMap((item) => (item.entries || []).map((entry) => <article key={entry.id}><strong>{item.product?.name} · round {entry.count_round} · {entry.entry_type}</strong><span>{entry.counted_quantity ?? "—"} · {entry.user?.name || "—"} · {entry.entered_at ? new Date(entry.entered_at).toLocaleString() : "—"}</span>{entry.notes && <p>{entry.notes}</p>}</article>))}</details>
      </> : <p>{t.empty}</p>}
    </div>
  </section>;
}
...
function ExpiryQueue({ data, meta, t }) {
  ...
  const expiryProducts = meta.products.filter((item) => item.tracking_mode === "batch_expiry");
...
function Catalogue({ data, meta, run, t, can }) {
  const blank = () => ({ product_id: "", supplier_id: "", supplier_sku: "", purchase_price: "", currency: "EUR", exchange_rate_to_base: 1, pack_size: 1, minimum_order_quantity: 0, usual_lead_time_days: 0, is_preferred: false, is_active: true, supplier_description: "", price_effective_at: new Date().toISOString().slice(0, 10), price_change_reason: "" });
  const [form, setForm] = useState(blank);
  const [editing, setEditing] = useState(null);
  const [performance, setPerformance] = useState(null);
  const [history, setHistory] = useState(null);
  const edit = (item) => { setEditing(item.id); setForm({ ...blank(), product_id: String(item.product_id), supplier_id: String(item.supplier_id), supplier_sku: item.supplier_sku || "", purchase_price: item.purchase_price ?? "", currency: item.currency || "EUR", exchange_rate_to_base: item.exchange_rate_to_base || 1, pack_size: item.pack_size || 1, minimum_order_quantity: item.minimum_order_quantity || 0, usual_lead_time_days: item.usual_lead_time_days || 0, is_preferred: Boolean(item.is_preferred), is_active: Boolean(item.is_active), supplier_description: item.supplier_description || "" }); };
  const submit = (event) => { event.preventDefault(); const values = { supplier_sku: form.supplier_sku || null, purchase_price: form.purchase_price === "" ? null : +form.purchase_price, currency: form.currency, exchange_rate_to_base: form.currency === "EUR" ? 1 : +form.exchange_rate_to_base, pack_size: +form.pack_size, minimum_order_quantity: +form.minimum_order_quantity, usual_lead_time_days: +form.usual_lead_time_days, is_preferred: form.is_preferred, is_active: form.is_active, supplier_description: form.supplier_description || null, price_effective_at: form.price_effective_at || null, price_change_reason: form.price_change_reason || null }; run(async () => { if (editing) await updateProductSupplier(editing, values); else await createProductSupplier({ ...values, product_id: +form.product_id, supplier_id: +form.supplier_id }); setEditing(null); setForm(blank()); }); };
  ...
  if (!can("supplier_catalogue.manage")) return <section className="ops-panel"><h2>{t.catalogue}</h2>{cards}<CatalogueInsights performance={performance} history={history} /></section>;
  return <section className="ops-layout"><aside className="ops-panel"><h2>{editing ? "Edit supplier offer" : "Add supplier offer"}</h2><form onSubmit={submit}><Field label="Product"><select required disabled={Boolean(editing)} value={form.product_id} onChange={(e) => setForm({ ...form, product_id: e.target.value })}><option value="">—</option>{meta.products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="Supplier"><select required disabled={Boolean(editing)} value={form.supplier_id} onChange={(e) => setForm({ ...form, supplier_id: e.target.value })}><option value="">—</option>{meta.suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field><Field label="Supplier SKU"><input value={form.supplier_sku} onChange={(e) => setForm({ ...form, supplier_sku: e.target.value })} /></Field><div className="ops-form-grid"><Field label="Price"><input type="number" min="0" step="0.000001" value={form.purchase_price} onChange={(e) => setForm({ ...form, purchase_price: e.target.value })} /></Field><Field label="Currency"><select value={form.currency} onChange={(e) => setForm({ ...form, currency: e.target.value, exchange_rate_to_base: e.target.value === "EUR" ? 1 : form.exchange_rate_to_base })}>{["EUR", "USD", "ALL", "GBP", "CNY"].map((currency) => <option key={currency}>{currency}</option>)}</select></Field>{form.currency !== "EUR" && <Field label="Rate to EUR"><input required type="number" min="0.00000001" step="0.00000001" value={form.exchange_rate_to_base} onChange={(e) => setForm({ ...form, exchange_rate_to_base: e.target.value })} /></Field>}<Field label="Pack size"><input type="number" min="0.001" step="0.001" value={form.pack_size} onChange={(e) => setForm({ ...form, pack_size: e.target.value })} /></Field><Field label="MOQ"><input type="number" min="0" step="0.001" value={form.minimum_order_quantity} onChange={(e) => setForm({ ...form, minimum_order_quantity: e.target.value })} /></Field><Field label="Lead days"><input type="number" min="0" value={form.usual_lead_time_days} onChange={(e) => setForm({ ...form, usual_lead_time_days: e.target.value })} /></Field></div><Field label="Supplier product description"><textarea value={form.supplier_description} onChange={(e) => setForm({ ...form, supplier_description: e.target.value })} /></Field><div className="ops-form-grid"><Field label="Price effective date"><input type="date" value={form.price_effective_at} onChange={(e) => setForm({ ...form, price_effective_at: e.target.value })} /></Field><Field label="Price change reason"><input value={form.price_change_reason} onChange={(e) => setForm({ ...form, price_change_reason: e.target.value })} /></Field></div><label className="ops-check"><input type="checkbox" checked={form.is_preferred} onChange={(e) => setForm({ ...form, is_preferred: e.target.checked })} /> Preferred supplier</label><label className="ops-check"><input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} /> Active</label><div className="ops-actions"><button>{t.save}</button>{editing && <button type="button" className="secondary" onClick={() => { setEditing(null); setForm(blank()); }}>Cancel edit</button>}</div></form></aside><div className="ops-panel"><h2>{t.catalogue}</h2>{cards}<CatalogueInsights performance={performance} history={history} /></div></section>;
}

function CatalogueInsights({ performance, history }) {
  return <>{history && <section className="ops-insight"><h3>{history.item.product?.name} · price history</h3><div className="ops-table"><table><thead><tr><th>Effective</th><th>Price</th><th>EUR value</th><th>Changed by</th><th>Reason</th></tr></thead><tbody>{history.rows.map((row) => <tr key={row.id}><td>{String(row.effective_at || "").slice(0, 10)}</td><td>{row.purchase_price} {row.currency}</td><td>{row.base_currency_price}</td><td>{row.changed_by?.name || "—"}</td><td>{row.change_reason || "—"}</td></tr>)}</tbody></table></div></section>}{performance && <section className="ops-insight"><h3>Supplier performance · {performance.supplier_name}</h3><div className="ops-metric-grid">{Object.entries(performance).filter(([key]) => !["supplier_id", "supplier_name"].includes(key)).map(([key, value]) => <div key={key}><span>{key.replaceAll("_", " ")}</span><strong>{value ?? "—"}</strong></div>)}</div></section>}</>;
}

function LandedCosts({ data, meta, run, t, can }) {
  const [form, setForm] = useState({ goods_receipt_id: "", cost_type: "freight", amount: "", currency: "EUR", allocation_method: "value", reference_number: "", description: "" });
  ...
  const manualTotal = Object.values(manualAmounts).reduce((sum, amount) => sum + (+amount || 0), 0);
  ...
  const submit = (event) => {
    ...
    const payload = { ...form, goods_receipt_id: +form.goods_receipt_id, amount: +form.amount, exchange_rate_to_base: 1, idempotency_key: uuid() };
...
function Returns({ data, meta, run, t, can }) {
  ...
  const payload = () => ({
    ...
    currency: "EUR", reference_number: form.reference_number || null, reason: form.reason, notes: form.notes || null,
    idempotency_key: uuid(), items: form.items.map((line) => { const product = meta.products.find((item) => String(item.id) === String(line.product_id)); const quantity = product?.tracking_mode === "serial" ? serials(line).length : +line.quantity; return {
  ...
  const submit = (event) => { event.preventDefault(); run(async () => { if (editing && selected) setSelected(await updateInventoryReturn(selected.id, payload())); else setSelected(await createInventoryReturn(payload())); setEditing(false); }); };
  ...
  return <section className={`ops-layout ${can("returns.manage") ? "" : "is-single"}`}>
    {can("returns.manage") && <aside className="ops-panel"><h2>{editing ? `Edit ${selected?.return_number}` : "New controlled return"}</h2><form onSubmit={submit}>
      ...
      <fieldset className="ops-lines"><legend>Return lines / Artikujt e kthimit</legend>{form.items.map((line, index) => { const product = meta.products.find((item) => String(item.id) === String(line.product_id)); const mode = product?.tracking_mode || "none"; const lineLocations = meta.locations.filter((location) => String(location.warehouse_id) === String(line.warehouse_id)); return <div className="ops-return-line" key={index}>
...
function SupplierMatching({ data, meta, run, t, can }) {
  ...
  const [allocation, setAllocation] = useState({ purchase_order_payment_id: "", amount: "", reason: "" });
  ...
  const open = async (id) => { setSelected(await getSupplierInvoice(id)); setEditing(null); setApprovalReason(""); setAllocation({ purchase_order_payment_id: "", amount: "", reason: "" }); };
  ...
  const paymentOptions = (selected?.purchase_order?.payments || []).map((payment) => ({ ...payment, available: availableForPayment(payment) })).filter((payment) => payment.available > 0.001);
